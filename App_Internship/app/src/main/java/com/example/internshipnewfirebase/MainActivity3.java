            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for(DataSnapshot sn:snapshot.getChildren()){
                    for(DataSnapshot sn1:sn.getChildren()){
                        ...
                        ar.add(st1);
                        data+=st1.getRollno()+" "+st1.getFname()+ ""+st1.getYear()+" "+st1.getTopic()+" "+st1.getPlatform()
                                +" "+st1.getStartdate()+" "+st1.getLastdate();
                    }
                }
                String[] arr=new String[ar.size()];
                int i=0;
                for (StudentsInfo st1:ar){
                    arr[i]=st1.getRollno()+" "+st1.getFname()+ ""+st1.getYear()+" "+st1.getTopic()+" "+st1.getPlatform()
                    +" "+st1.getStartdate()+" "+st1.getLastdate();
                    i++;
                }
